...
rothsmithZone=$(aws route53 list-hosted-zones-by-name --dns-name "${loadBalancerPrefix}.rothsmith.net" --max-items 1 --query 'HostedZones[*].Id' --output text | sed 's/\/hostedzone\///g')
